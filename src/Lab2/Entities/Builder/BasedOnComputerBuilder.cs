...
public class BasedOnComputerBuilder
{
    private BIOS? _bios;
    ...
    public BasedOnComputerBuilder(Computer computer)
    {
    ...
    {
        if (gpu is not null && !_cpu.InternalGPU && gpu.Height <= _case.MaxGPULength)
        {
            _gpu = gpu;
        }
        else
    ...
    public void MotherBoard(MotherBoard motherBoard)
    {
        ...
        if (motherBoard.Socket == _cpu.SocketName && motherBoard.BIOS.SupportedCPUs.Contains(_cpu.SocketName) &&
            _case.SupportedMotherBoardFormFactor.Contains(motherBoard.MotherBoardFormFactor))
        {
            _motherBoard = motherBoard;
        }
        else
        {
            StatusOfBuilding = Status.FailWithMotherBoard;
        }
    }
